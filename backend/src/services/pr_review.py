    ...
    def from_request(
        ...
        return cls(
            ...
            requirements=(
                ...
                else (stored.get("requirements") or [])
            ),
            user_id=user_id,
        )
...
def _graph_block(tree: RepoTree) -> str:
    ...
    return (
        f"Graph: {g.nodes_written} nodes, {g.relationships_written} rels. "
        f"Console: {g.console_url}"
    )
...
async def review_pr(
    token: str, pr: PRContext, ctx: RepoContext
) -> tuple[PRVerdict, RepoTree | None]:
    ...
    try:
        raw = await complete(
            [{"role": "system", "content": _SYSTEM}, {"role": "user", "content": human}],
            json_mode=True,
        )
...
def render_comment(
    pr: PRContext, v: PRVerdict, ctx: RepoContext, graph_url: str | None
) -> str:
    layers = [
        "✅ Requirements" if ctx.requirements else "⚪ Requirements",
        (
            f"✅ DOM/UI ({ctx.crawl.get('screen_count', 0)} screens)"
            if ctx.crawl
            else "⚪ DOM/UI"
        ),
